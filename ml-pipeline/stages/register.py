...
def get_production_f1(model_name: str) -> float:
    ...
    try:
        prod = client.get_model_version_by_alias(model_name, REG_PARAMS["production_alias"])
...
def main() -> None:
    ...
    train_run = next((r for r in runs if r.data.tags.get("mlflow.runName") == "train"), runs[0])
    model_uri = f"runs:/{train_run.info.run_id}/model"

    mv = mlflow.register_model(model_uri, model_name)
    client.set_registered_model_alias(model_name, REG_PARAMS["staging_alias"], mv.version)
    print(f"Registered version {mv.version} as '{REG_PARAMS['staging_alias']}'")

    # Auto-promote staging to production when F1 gate passed
    client.set_registered_model_alias(model_name, REG_PARAMS["production_alias"], mv.version)
